// Reconnect mid-turn: opening a session whose turn is already running server-side never
// sees a live `turn_start`. `running` must be restored from the ws `ready` payload.
// Otherwise the Stop button and the "Waiting for agent" row vanish and the user cannot
// stop the turn.
