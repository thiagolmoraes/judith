      ...
      <div className="flex-1 overflow-y-auto hairline-scroll">
        <div className="max-w-3xl mx-auto px-7 py-6 space-y-6">
          ...
          {detail.tools.length > 0 && (
            <section>
              <div className={`${SEC_H} mb-2`}>{t("persona.builtinCapabilities")}</div>
          ...
          {detail.recommends.length > 0 && (
            <section>
              <div className={`${SEC_H} mb-1`}>{t("persona.connectionsForBenefit")}</div>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {isMcp ? (
                            <span className={TAG_MCP}>{t("persona.mcp")}</span>
                          ) : r.tier === "core" ? (
          ...
          {detail.default_connections.length > 0 && (
            <section>
              <div className={`${SEC_H} mb-1`}>{t("persona.newSessionsDefault")}</div>
              <p className="text-[12.5px] text-muted mb-2.5">
          ...
          <section className="flex flex-wrap gap-x-8 gap-y-2 text-[12.5px]">
            {detail.recommended_models.length > 0 && (
              <div>
                <span className="text-faint">{t("persona.models")}</span> ·{" "}
            ...
            {detail.default_permission_mode && (
              <div>
                <span className="text-faint">{t("persona.defaultMode")}</span> · {detail.default_permission_mode}
              </div>
            )}
            {detail.workspace && (
              <div>
                <span className="text-faint">{t("persona.workspace")}</span> · {detail.workspace}
              </div>
            )}
          </section>
        </div>
