...
// The standing grants "Always allow (permanent)" mints on approval cards, in the three
// scopes the server routes them into: tool-wide, exact shell command, per-target.
// Remove re-arms the question — the next matching action simply asks again.
  ...
  return (
    <div className="mt-3">
      ...
      <div className="divide-y divide-line">
        {rows.map((row) => (
          <div key={row.key} className="py-2 flex items-center gap-3">
...
function ApprovalsCard() {
  ...
  return (
    <div className={CARD + " p-4 mb-4"} data-testid="approvals-card">
      <div className={FIELD_LABEL}>{t("settings.approvals.title")}</div>
      {snap === null ? (
        <div className="text-[12px] text-muted mt-3">{t("common.loading")}</div>
      ) : empty ? (
        <div className="text-[12px] text-muted mt-3">{t("settings.approvals.empty")}</div>
      ) : (
        <>
          <ApprovalGroup
