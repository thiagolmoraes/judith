  ...
  return (
    <div className="relative flex-1 min-w-0" ref={wrap}>
      <input
        ...
        placeholder={t("subs.channelPlaceholder")}
        value={display}
    ...
    <div className="sub-chip-wrap" ref={ref}>
      ...
      {open && (
        <div className="sub-pop" onMouseDown={(e) => e.stopPropagation()}>
          <div className="sub-pop-head">{t("subs.channelsHeading")}</div>
          {channels.length === 0 ? (
            <div className="dim sub-pop-empty">{t("subs.notSubscribed")}</div>
          ) : (
            channels.map((c) => {
