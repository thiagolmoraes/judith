  ...
  return (
    <div data-testid="github-installations">
      ...
      {relay && listening.length > 0 && (
        <>
          <div className={GRP_H}>{t("conn.listening")}</div>
          <div className={GRP}>
      ...
      {adding && (
        <AddConnectionModal
          c={c}
          cloud={cloud}
          title={t("github.addInstallation")}
  ...
  return (
    <div className={ROW}>
      ...
      <span className="min-w-0 flex-1 flex flex-wrap items-center gap-1.5">
        {allowed.length === 0 && (
          <span className="text-[12px] text-faint">{t("github.nobodyYet")}</span>
        )}
        {allowed.map((login) => (
          <span
            key={login}
  ...
  return (
    <div className={ROW + " bg-warnSoft/25"} data-testid={`waiting-${m.id}`}>
      ...
      <button
        ...
        title={t("github.allowDeliverTitle")}
