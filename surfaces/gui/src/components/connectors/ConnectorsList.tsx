  ...
  return (
    <div>
      ...
      <div className={GRP}>
        ...
        {shown.length === 0 && (
          <div className={ROW + " text-[12.5px] text-muted"}>{t("connectors.nothingMatches")}</div>
        )}
      </div>
...
function statusLine(c: Connector, t: Translate): string {
  if (c.name === "slack" && c.mode === "relay") {
    const n = c.workspaces?.length ?? 0;
    return t("connectors.relayLine", {
      workspaces: t("count.workspaces", { count: n }),
    });
  }
  if ((c.accounts?.length ?? 0) > 1)
    return t("connectors.accounts", { count: c.accounts!.length });
  if ((c.portals?.length ?? 0) > 1)
    return t("connectors.portals", { count: c.portals!.length });
  if (c.auth === "none") return t("connectors.builtIn");
