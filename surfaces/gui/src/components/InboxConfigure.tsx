  ...
  return (
    <div data-testid="inbox-configure">
      ...
      <div className="mt-6" data-testid="unrouted-section">
        <h3 className="text-[14px] font-semibold mb-1">{t("inbox.unrouted")}</h3>
        <p className="text-[12.5px] text-muted mb-3">
          {t("inbox.unroutedHelp")}
        </p>
        <UnroutedTable />
      </div>
    </div>
...
function InboxRoutingCard() {
  ...
  const save = async () => {
    ...
    if (!result.ok) {
      setError(result.error || t("inbox.couldNotUpdate"));
      return;
    }
    setError(null);
    setDraft("");
  ...
  const clear = async () => {
    ...
    if (!result.ok) {
      setError(result.error || t("inbox.couldNotClear"));
      return;
    }
    setError(null);
    load();
  ...
  return (
    <div className={CARD + " p-4"} data-testid="inbox-mirror-card">
      <div className="font-semibold text-[13.5px] mb-1">{t("inbox.unattendedApprovals")}</div>
      <p className="text-[12px] text-muted mb-3">
        {t("inbox.mirrorHelp", {
          target: known ? `#${known}` : target || t("inbox.inAppOnly"),
        })}
      </p>
...
function DmRouteCard() {
  ...
  return (
    <div className={CARD + " p-4"}>
      <div className="font-semibold text-[13.5px] mb-1">{t("inbox.directMessages")}</div>
      <p className="text-[12px] text-muted mb-3">
        {t("inbox.dmHelp")}
      </p>
      <div className="flex items-center gap-2">
        ...
        <select className={"flex-1 " + SELECT} value={dm} onChange={(e) => choose(e.target.value)}>
          <option value="">{t("inbox.noSessionPark")}</option>
          {real.map((s) => (
  ...
  return (
    <div className={CARD + " mb-4 overflow-hidden"}>
      <div className="px-4 py-3 border-b border-line flex items-center gap-2">
        ...
        <span className="font-semibold text-[13.5px]">{t("inbox.subscriptions")}</span>
        <span className="text-[12px] text-muted">{t("inbox.subscriptionsHint")}</span>
      </div>
        ...
        <table className="w-full text-[13px]">
          <thead className="text-[11px] uppercase tracking-[0.04em] text-faint">
            <tr className="text-left">
              <th className="font-medium px-4 py-2">{t("inbox.colSession")}</th>
              <th className="font-medium px-4 py-2">{t("inbox.colListensTo")}</th>
              <th className="font-medium px-4 py-2">{t("inbox.colRoutesTo")}</th>
      ...
      <div className="border-t border-line px-4 py-3 flex items-center gap-2 flex-wrap">
        ...
        >
          <option value="">{t("inbox.chooseSession")}</option>
          {real.map((s) => (
            <option key={s.session_id} value={s.session_id}>
              {s.title || s.session_id}
            </option>
  ...
  return (
    <div className={CARD + " overflow-hidden"}>
      <table className="w-full text-[13px]">
        <thead className="text-[11px] uppercase tracking-[0.04em] text-faint">
          <tr className="text-left">
            <th className="font-medium px-4 py-2">{t("inbox.colWhen")}</th>
            <th className="font-medium px-4 py-2">{t("inbox.colSource")}</th>
            <th className="font-medium px-4 py-2">{t("inbox.colReason")}</th>
            <th className="font-medium px-4 py-2">{t("inbox.colMessage")}</th>
          </tr>
        </thead>
        <tbody>
          {(items ?? []).map((it, i) => (
