...
def _browser_page(
    ...
    return (
        ...
        f"<title>{_html.escape(title)} — Judith</title><style>"
        ...
        "</style></head><body>"
        '<div class="card"><div class="mark"><i></i>Judith</div>'
...
def create_app(manager: SessionManager) -> FastAPI:
    ...
    async def require_sidecar_token(request: Request, call_next):
        ...
        return JSONResponse(
            {"error": "missing or invalid Judith sidecar token"},
            status_code=401,
        )
    ...
    # -- Judith Cloud: sign-in + managed one-click connect ---------------
        ...
        await ws.send_json(
            {
                ...
                "data": {
                    ...
                    # drop. The GUI never sees that turn's turn_start. Without this flag
                    # it shows no Stop button and no waiting row.
